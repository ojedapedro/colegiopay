...
const VerificationList: React.FC<Props> = ({ payments, representatives, fees, onVerify, onImportExternal }) => {
  ...
  const handleSyncVirtualOffice = async () => {
    ...
    try {
      ...
      if (externalPayments && externalPayments.length > 0 && onImportExternal) {
        ...
        alert("ℹ️ No se detectaron pagos nuevos en la Oficina Virtual.");
      }
    } catch (e) {
      console.error(e);
  ...
  const getRepresentativeStats = (cedula: string) => {
    ...
    if (!rep) return { currentBalance: 0, name: 'ID NO VINCULADO', matricula: 'SIN MATRÍCULA' };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-1 bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-8">
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Filtros</h4>
          </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/80 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                    ...
                    <th className="px-8 py-5">Instrumento / Ref</th>
                    <th className="px-8 py-5 text-center">Abono / Saldo</th>
                      ...
                      <tr key={p.id} className="group hover:bg-slate-50/60 transition-all">
                        <td className="px-8 py-7">
                          <div className="flex flex-col gap-1">
                            ...
                            <span className="text-[9px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 w-fit">{matricula}</span>
                          </div>
                        </td>
                        <td className="px-8 py-7">
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="text-[10px] font-mono font-bold text-slate-600">REF: {p.reference}</div>
                            <span className="text-[9px] font-black text-slate-400">{p.paymentDate}</span>
                          </div>
                        </td>
                        <td className="px-8 py-7 text-center">
                          <div className="flex flex-col items-center gap-1">
                            ...
                            <div className="flex items-center gap-1 text-[9px] font-bold text-slate-400">
                              ...
                              <span className="text-emerald-600">${projectedBalance.toFixed(2)}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-8 py-7">
